...
export class MongoDatabase implements Database {
  ...
  private taskCollection: Collection<Document>;
  ...
  public async updateUser(user_id: string, update: any): Promise<UserResponse> {
    const result = await this.userCollection.updateOne({ '_id': new ObjectId(user_id) }, update);
  ...
  public async addTask(task: Task): Promise<TaskResponse> {
    ...
    const userResult = await this.updateUser(
      task.user_id,
      new UserUpdateBuilder().addTask(result.insertedId.toString()).getUpdate()
    );
  ...
  public async updateTask(task_id: string, update: any): Promise<TaskResponse> {
    const result = await this.taskCollection.updateOne({ '_id': new ObjectId(task_id) }, update);
    if (!result.acknowledged || result.matchedCount === 0 || result.modifiedCount === 0) {
